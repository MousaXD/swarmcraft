...
function moduleLoaderExpression(graph) {
  return `(async () => {
    const modules = ${JSON.stringify(graph)};
    const urls = new Map();
    ...
    const entry = await build(${JSON.stringify(entryModule)});
    await import(entry);
...
async function runBrowser(browser, documentHtml, graph, width, height, screenshotPath) {
  ...
  try {
    ...
    await client.send('Page.setDocumentContent', { frameId: frameTree.frame.id, html: documentHtml }, sessionId);
    await client.send('Runtime.evaluate', { expression: tauriFixture }, sessionId);

    const loaded = await client.send('Runtime.evaluate', {
      expression: moduleLoaderExpression(graph), returnByValue: true, awaitPromise: true,
    }, sessionId, 20000);
    if (loaded.exceptionDetails) throw new Error(loaded.exceptionDetails.text || 'Desktop module graph failed to import');
    assert.ok(loaded.result?.value?.modules >= 2, 'Browser smoke did not load the Desktop module graph');

    const evaluated = await client.send('Runtime.evaluate', {
      expression: scenario, returnByValue: true, awaitPromise: true,
    }, sessionId, 20000);
...
test('current Desktop module graph initializes, owns canonical Create, and renders supported window sizes', async () => {
  ...
  for (const [width, height] of [[980, 760], [720, 560], [1280, 900]]) {
    ...
    await runBrowser(browser, documentHtml, graph, width, height, screenshot);
