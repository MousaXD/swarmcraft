...
async function waitForTarget(port, timeoutMs = 10000) {
  ...
  throw new Error(`Chromium DevTools target did not become available: ${lastError || 'timeout'}`);
}
...
class DevToolsClient {
  ...
  async connect() {
    ...
    this.socket.addEventListener('close', () => {
      for (const pending of this.pending.values()) pending.reject(new Error(`DevTools socket closed during ${pending.method}`));
      this.pending.clear();
    });
    await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error('DevTools WebSocket connection timed out')), 5000);
...
async function waitForSmokeResult(client, timeoutMs = 10000) {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    const status = await evaluate(client, `document.documentElement?.dataset?.browserSmoke || ''`);
...
async function runBrowser(browser, url, width, height, screenshotPath) {
  ...
  const client = new DevToolsClient((await waitForTarget(port)).webSocketDebuggerUrl);
  try {
    ...
    const status = await waitForSmokeResult(client);
    if (status !== 'pass') {
    ...
    assert.equal(await evaluate(client, `document.documentElement.dataset.layoutPass`), 'true');
