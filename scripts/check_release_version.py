...
if cargo.get("workspace", {}).get("package", {}).get("version") != EXPECTED_APP:
    fail("workspace.package.version is not 0.5.0")
...
if desktop.get("package", {}).get("version") != EXPECTED_APP:
    fail("desktop Cargo.toml package.version is not 0.5.0")
...
if tauri.get("version") != EXPECTED_APP:
    fail("Tauri application version is not 0.5.0")

gradle = (ROOT / "minecraft/fabric/gradle.properties").read_text(encoding="utf-8")
if not re.search(r"^mod_version=0\.5\.0$", gradle, re.MULTILINE):
    fail("Fabric mod_version is not 0.5.0")
