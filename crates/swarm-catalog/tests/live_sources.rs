...
fn official_mojang_and_fabric_sources_are_compatible() {
    ...
    let minecraft = service.minecraft_versions(false, true).expect("live Mojang release catalog");
    ...
    for version in minecraft.versions.iter().take(5) {
        let loaders = service.fabric_loader_versions(&version.id, true).expect("live Fabric loader catalog");
        if !loaders.versions.is_empty() {
            assert!(loaders.versions.iter().all(|loader| loader.minecraft_version == version.id));
            found_compatible_loader = true;
            break;
        }
    }
    assert!(found_compatible_loader, "Fabric Meta returned no loaders for the five newest Mojang releases");
}
