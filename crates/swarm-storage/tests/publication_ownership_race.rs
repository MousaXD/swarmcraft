...
fn spawn_publisher(
    ...
    thread::spawn(move || {
        ...
        commit.wait();
        storage.commit_snapshot_streaming(&publication).expect("local publication commit should succeed");
    })
}
...
fn simultaneous_local_publishers_survive_replica_commit_gc_and_retention() {
    ...
    let first =
        spawn_publisher(storage.clone(), &shared_source, world, 1, start.clone(), commit.clone(), published_tx.clone());
    let second =
    ...
    assert_eq!(
        ...
        "shared blob must remain live only because both local publication owners pin it"
    );
