...
use std::{collections::BTreeSet, fs::{self, File, OpenOptions}, io::{Read, Seek, SeekFrom, Write}, path::{Path, PathBuf}};
...
impl Storage {
    ...
    pub fn read_encoded_blob_chunk(&self, world: WorldId, descriptor: &BlobDescriptor, offset: u64, max_bytes: usize) -> Result<(Vec<u8>, bool), ReplicationError> {
        ...
        if max_bytes == 0 { return Ok((Vec::new(), offset == descriptor.encoded_size)); }
        let path = self.blob_path(world, descriptor.hash, descriptor.encoding);
        let metadata = fs::metadata(&path).map_err(|error| io_error(&path, error))?;
        if metadata.len() != descriptor.encoded_size { return Err(StorageError::BlobCorrupt(descriptor.hash).into()); }
    ...
    pub fn receive_blob_chunk(&self, world: WorldId, descriptor: &BlobDescriptor, offset: u64, data: &[u8], finished: bool) -> Result<u64, ReplicationError> {
        ...
        let mut file = OpenOptions::new().create(true).read(true).write(true).open(&partial).map_err(|error| io_error(&partial, error))?;
        let current = file.metadata().map_err(|error| io_error(&partial, error))?.len();
        if current != offset { return Err(ReplicationError::OffsetMismatch { expected: current, received: offset }); }
        let next = current.saturating_add(data.len() as u64);
        if next > descriptor.encoded_size { return Err(ReplicationError::SizeMismatch { expected: descriptor.encoded_size, received: next }); }
        ...
        if finished {
            ...
            if final_path.exists() { fs::remove_file(&final_path).map_err(|error| io_error(&final_path, error))?; }
    ...
    pub fn finalize_replica(&self, manifest: &SnapshotManifestV1) -> Result<(), ReplicationError> {
        if !self.missing_blobs(manifest).is_empty() { return Err(ReplicationError::Incomplete(manifest.snapshot_number)); }
        self.commit_snapshot(manifest)?;
        Ok(())
    }

    fn partial_blob_path(&self, world: WorldId, descriptor: &BlobDescriptor) -> PathBuf {
        let suffix = match descriptor.encoding { BlobEncoding::Raw => "raw", BlobEncoding::Zstd => "zst" };
...
fn verify_encoded_blob_path(path: &Path, descriptor: &BlobDescriptor) -> Result<(), ReplicationError> {
    ...
    let mut reader: Box<dyn Read> = match descriptor.encoding {
        ...
        BlobEncoding::Zstd => Box::new(zstd::stream::read::Decoder::new(file).map_err(|_| StorageError::BlobCorrupt(descriptor.hash))?),
    };
