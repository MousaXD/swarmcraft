...
fn prepare_proposal(authority: &PeerFixture, current: &MembershipRecordV1, proposed_members: Vec<WorldMemberV1>) {
    ...
    let mut vote = membership_vote_for(&proposal, authority.identity.peer_id(), authority.identity.public_key()).unwrap();
...
fn assert_unclean_quorum_loss_does_not_enter_solo(member_count: usize) {
    ...
    wait_until("old authority permit removal", Duration::from_secs(12), || permit(refs[0], metadata.world_id).is_none());
    ...
    wait_until("majority recovery permit", Duration::from_secs(35), || {
        majority.iter().any(|peer| {
            permit(peer, metadata.world_id).is_some_and(|(ep, fence, _)| ep == 2 && fence == 2)
        })
    });
